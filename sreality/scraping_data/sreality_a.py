...
referer = {'prodej_byty': 'https://www.sreality.cz/hledani/prodej/byty/praha',
           'pronajem_byty': 'https://www.sreality.cz/hledani/pronajem/byty/praha'}

apartments_room_size = ["pokoj", 
                        "1%2Bkk","1%2B1",
                        "2%2Bkk","2%2B1",
                        "3%2Bkk","3%2B1",
                        "4%2Bkk","4%2B1",
...
apartments_room_size_correct_key = None
...
def rent_or_buy():
    ...
    if(sys.argv[2] == "buy"):
        num = 1
        ref = referer['prodej_byty']
    else: # rent
        num = 2
        ref = referer['pronajem_byty']
    #print(num)
...
def apartments_room_sizeKey(roomCount):
    if(roomCount == 2): #1+kk
        velikost_pokoju = apartments_room_size[1]
        apartments_room_size_correct_key = 2
    elif(roomCount == 3): # 1+1
        velikost_pokoju = apartments_room_size[2]
        apartments_room_size_correct_key = 3
    elif(roomCount == 4): # 2+kk
        velikost_pokoju = apartments_room_size[3]
        apartments_room_size_correct_key = 4
    elif(roomCount == 5): #2+1
        velikost_pokoju = apartments_room_size[4]
        apartments_room_size_correct_key = 5
    elif(roomCount == 6): #3+kk
        velikost_pokoju = apartments_room_size[5]
        apartments_room_size_correct_key = 6
    elif(roomCount == 7): # 3+1
        velikost_pokoju = apartments_room_size[6]
        apartments_room_size_correct_key = 7
    elif(roomCount == 8): #4+kk
        velikost_pokoju = apartments_room_size[7]
        apartments_room_size_correct_key = 8
    elif(roomCount == 9): #4+1
        velikost_pokoju = apartments_room_size[8]
        apartments_room_size_correct_key = 9
    elif(roomCount == 10): # 5+kk
        velikost_pokoju = apartments_room_size[9]
        apartments_room_size_correct_key = 10
    elif(roomCount == 11): # 5+1
        velikost_pokoju = apartments_room_size[10]
        apartments_room_size_correct_key = 11
    elif(roomCount == 12): # 6+
        velikost_pokoju = apartments_room_size[11]
        apartments_room_size_correct_key = 12
    else: #16 = atypicky
        velikost_pokoju = apartments_room_size[12]
        apartments_room_size_correct_key = 16
    
    if(sys.argv[2] == "rent"):
        if(roomCount == 1): # room for rent
            velikost_pokoju = apartments_room_size[0]
            apartments_room_size_correct_key = 47
    
    
    return(velikost_pokoju, apartments_room_size_correct_key)
...
def create_single_header_params(roomCount):
    ...
    velikost_pokoju, apartments_room_size_correct_key = apartments_room_sizeKey(roomCount)
    ...
    'Cookie': 'lastsrch="{\"category_main_cb\": \"1\"\054 \"per_page\": \"60\"\054 \"tms\": \"' + str(generateTimeEpoch()) + '\"\054 \"locality_region_id\": \"10\"\054 \"category_sub_cb\": \"' + str(apartments_room_size_correct_key) +'\" \054 \"category_type_cb\": \"' + str(id_number_rent_buy) + '\"}"',
...
def store_and_returnHashIDs(roomCount, per_page = 60, page = number_of_listings_to_download()):
    ...
    for i in range(1,int(page)+1):
        params = {
            ...
            'category_sub_cb': str(apartments_room_size_correct_key),
...
def buy_rent_individual_apartments_DF(roomCount):
    ...
    #labels = None
    for ids, apartment_number_url in urls_list:

        random_number_forSQL_table = random.randint(0,999999999999999)

        response = requests.get(apartment_number_url, params = req_params, headers = create_single_header_params(roomCount))
        ...
        print("we are here processing: -> " + str(apartment_number_url))
        ...
        descriptionRows = [(random_number_forSQL_table, time_snapshot, ids, apartment_number_url, name_value, description_value,
    ...
    df.to_csv("/home/jm/Documents/master_new_repo/code_data/scraping_data/apartments_ind_buy_and_rent.csv", mode='a')
    print("Done with all")

    return(df)


def store_in_mariaDB(room_count_type = range(1,14)):
    """
    ...
    """
    engine = create_engine('mysql+pymysql://root:@localhost/sreality_cz?charset=utf8mb4', encoding='utf8', echo = True)
    
    for i in room_count_type:
        #print(i)
        ind_houses = buy_rent_individual_apartments_DF(roomCount = i)

        if(sys.argv[2] == "buy"):
            ind_houses.to_sql('individual_apartments_toBuy_roomSize_'+str(i), engine, if_exists='append', 
            ...
            ind_houses.to_sql('individual_apartments_toRent_roomSize_'+str(i), engine, if_exists='append', 
                          dtype={'swimming_pool': sqlalchemy.types.Text, 'elevator': sqlalchemy.types.Text,
                                'equipment': sqlalchemy.types.Text, 'parking': sqlalchemy.types.Text,
                                'cellar': sqlalchemy.types.Text, 'garage': sqlalchemy.types.Text,
                                'non_barrier': sqlalchemy.types.Text, 'random_id_num': sqlalchemy.types.Text, 
...
def main():
    #print(generate_listing_URLs(roomCount = 5))
...
if __name__ == "__main__":
    ...
    print("Rent or buy? Which are choosen? --> " + sys.argv[2])
    print("   ")
    main()
